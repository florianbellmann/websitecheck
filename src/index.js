...
function getValidDates(today) {
  ...
  const dates = [today, yesterday, twodaysago, threedaysago];
  return [
    getDateString(today),
  ...
  resultDirectories.forEach((element) => {
    if (validDates.indexOf(element) < 0) {
      try {
        ...
        deleteFolderRecursive(path.join(resultLocation, element))
      } catch (error) {
        console.error(error);
      }
    }
...
async function compareImage(siteName, dateString1, dateString2) {
  return new Promise(async (resolve) => {
    const pixelDifference = await getPixelDifference(
      siteName,
      dateString1,
      dateString2
    );
      ...
      if (outerError) {
        try {
          ...
          if (stderr.indexOf("no such file") > 0) {
            ...
            console.error(
              "Could not parse pixel difference.",
              stdout,
              innerError,
              outerError
            );
...
(async () => {
  ...
  if (
    !fs.existsSync(
      path.join(resultLocation, getDateString(dateHelper.getDayBefore(today)))
    )
  ) {
